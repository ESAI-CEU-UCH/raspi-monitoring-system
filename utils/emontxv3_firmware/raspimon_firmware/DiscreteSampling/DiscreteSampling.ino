/*
  ...
  Original output when using default firmware:
...
Port /dev/ttyUSB0, 13:54:09

Presione CTRL-A Z para obtener ayuda sobre teclas especiales

emonTx V3.4 Discrete Sampling V2.10 RFM69CW
...
const long TIME_BETWEEN_READINGS = 1000;            // Time between readings in ms
...
void loop()
{
  ...
  unsigned long sleeptime = 10;
  // raspimon: added if to avoid overflow time computation
  if (runtime + 100 < TIME_BETWEEN_READINGS) sleeptime = TIME_BETWEEN_READINGS - runtime - 100;
  
  if (debug == 1) {
    Serial.print("runtime: "); Serial.println(runtime);
    Serial.print("sleep for (miliseconds): "); Serial.println(sleeptime);
  }
  ...
  if (ACAC) {                                                               // If powered by AC-AC adaper (mains power) then delay instead of sleep
    ...
    if (sleeptime > 500) Sleepy::loseSomeTime(sleeptime - 500);                                    // sleep or delay in seconds
    else Sleepy::loseSomeTime(10);
  }
}
